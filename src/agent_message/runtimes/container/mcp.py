"""MCP server exposing the approved container execution tool."""
...
from .policy import CONTAINER_INSTRUCTIONS
from .runner import ContainerRunnerError, run_container


SERVER_NAME = "agent-message-container"
SERVER_VERSION = "0.1.0"
INSTRUCTIONS = CONTAINER_INSTRUCTIONS
...
class ContainerMcpServer:
    def __init__(self, config_path: str, task_id: str, run_id: int | None) -> None:
        ...
        if self.project.container is None:
            raise ConfigError(
                f"container execution is not enabled for project {self.project.alias}"
            )
        ...
        return [
            {
                "name": "container_run",
                "description": (
                    "Run an argv command inside the current project's configured existing "
                    "Docker container and bind-mounted project directory."
                ),
                "inputSchema": {
                    "type": "object",
                    "properties": {
                ...
                "annotations": {
                    ...
                    # docker exec inherits the configured container network.
                    "openWorldHint": True,
                },
            }
        ]
    ...
    def _run_tool(self, request_id: object, arguments: dict[str, Any]) -> None:
        ...
        if not acquired:
            self._tool_response(
                request_id,
                {"error": "another container command is already running in this agent process"},
                is_error=True,
            )
            with self._requests_lock:
        ...
        try:
            ...
            if not isinstance(cwd, str):
                raise ContainerRunnerError("cwd must be a string")
            container = self.project.container
            assert container is not None
            job = self.state.create_container_job(
            ...
            result = run_container(
                ...
                cwd,
                job.log_path,
                on_pid=lambda pid: self.state.set_container_job_pid(job.id, pid),
                on_container_pid=lambda pid: self.state.set_container_job_inner_pid(
                    job.id, pid
                ),
                cancel_event=cancel,
            )
            final = self.state.finish_container_job(
                job.id,
            ...
            self._tool_response(
                request_id,
                payload,
                is_error=result.exit_code != 0 or result.error is not None,
            )
        except (ContainerRunnerError, ConfigError, ValueError) as exc:
            if job is not None:
                self.state.finish_container_job(job.id, exit_code=1, error=str(exc))
            self._tool_response(request_id, {"error": str(exc)}, is_error=True)
        except Exception as exc:
            if job is not None:
                self.state.finish_container_job(
                    job.id, exit_code=1, error=f"unexpected error: {exc}"
                )
            self._tool_response(
                request_id,
                {"error": f"unexpected container runner error: {exc}"},
                is_error=True,
            )
        finally:
    ...
    def _dispatch(self, message: dict[str, Any]) -> None:
        ...
        if method == "tools/call" and "id" in message:
            if not isinstance(params, dict) or params.get("name") != "container_run":
            ...
            worker = threading.Thread(
                ...
                name=f"agent-message-container-{request_id}",
                daemon=True,
            )
