...
def load_config(path: str | Path) -> AppConfig:
    ...
    for alias, project_raw in projects_raw.items():
        ...
        projects[alias] = ProjectConfig(alias, project_path, default, allowed)
    ...
    if AgentKind.CODEX not in default_project.allowed_agents:
        raise ConfigError(
            f"[service].default_chat_project ({default_chat_project}) must allow codex"
        )
